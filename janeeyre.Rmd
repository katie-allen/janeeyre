...
You may ask, *"Katie, how did you know to use 8 on the low-pass filter?"* Great question. I tried everything from 2 to 10 and decided 8 looked the best and was the easiest to explain.

```{r}
jane_dct <- as.numeric(get_dct_transform(jane_sentiment$sentiment, low_pass_size = 8, x_reverse_len = 100, scale_vals = T, scale_range = F))
...
Beautiful! We can see Jane's horrible childhood, the happy early days at Thornfield, the complete despair when the wedding is broken off, and...a sad ending?

### Analysis by Chapter

*Jane Eyre* does have a happy ending, but the tragic events immediately preceding the last few chapters (combined with behavior issues at edge points) show the book ending on a low note.

Just for fun, let's look at the 8 NRC emotions by chapter and see if we can find the happily ever after.

```{r}
jane_chapters <- data.frame("chapter"=1:38, "text"= rep("NA", 38))
...
The brightest spot is **fear** in Chapter 27. This the chapter after Jane's wedding is canceled and as we saw before, it is a low point in her story. She loses the love of her life, runs away from the only home she's ever known, and nearly starves to death.

Sounds about right.

Something to consider here is word count. Chapter 27 may have the strongest emotion, but it also has the most words at 11257! The final chapter has only 1817 words. What happens when we normalize the emotion scores?
...
levels(emotions2$sentiment) <- c("Anger", "Anticipation", "Disgust", "Fear", "Joy", "Sadness", "Surprise", "Trust")
...
Finally, the happy ending comes shining through! The brightest emotion is now **trust** in the final chapter. Jane is given a fortune, reunited with Mr. Rochester, and surrounds herself with the people she loves. 

The second strongest emotion is **fear** in Chapter 2, when 10-year-old Jane is locked in a haunted room and faints from sheer terror. It is interesting that, when accounting for word count, the emotions of this childhood event surpass any anger, fear, or sadness that Jane experiences for the rest of the novel. 
